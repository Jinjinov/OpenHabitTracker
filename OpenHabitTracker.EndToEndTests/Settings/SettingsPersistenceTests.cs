...
public class SettingsPersistenceTests : BaseTest
{
    ...
    public async Task Language_Change_AppliesImmediately()
    {
        ...
        // data-settings-step-18 wraps the language select
        await Page.Locator("[data-settings-step-18] select").SelectOptionAsync("de");
    ...
    public async Task Language_Change_PersistedAfterReload()
    {
        ...
        await OpenSettingsAsync();
        await Page.Locator("[data-settings-step-18] select").SelectOptionAsync("en");
    }
    ...
    [Test]
    public async Task NotificationHour_Change_PersistedAfterReload()
    {
        ...
        await Context.GrantPermissionsAsync(["notifications"]);

        await OpenSettingsAsync();

        await Page.Locator("[data-settings-step-27] select").SelectOptionAsync("9");

        await WaitForIndexedDbAsync("SettingsEntity", "settings => settings.some(s => (s.notificationHour ?? s.NotificationHour) === 9)");

        await Page.ReloadAsync();
        await OpenSettingsAsync();

        await Expect(Page.Locator("[data-settings-step-27] select")).ToHaveValueAsync("9");
    }
    ...
    public async Task NotificationLeadTime_Change_PersistedAfterReload()
    {
        ...
        await Page.Locator("[data-settings-step-28] select").SelectOptionAsync("30");

        await WaitForIndexedDbAsync("SettingsEntity", "settings => settings.some(s => (s.notificationLeadMinutes ?? s.NotificationLeadMinutes) === 30)");

        await Page.ReloadAsync();
        await OpenSettingsAsync();

        await Expect(Page.Locator("[data-settings-step-28] select")).ToHaveValueAsync("30");
    }

    [Test]
    public async Task NotificationHour_WhenPermissionIsRefused_GoesBackToOff()
    {
        ...
        await Expect(Page.Locator("[data-settings-step-27] select")).ToHaveValueAsync("-1");
    }
}
