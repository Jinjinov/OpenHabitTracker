...
public sealed class JsInterop(IJSRuntime jsRuntime) : IJsInterop, IAsyncDisposable
{
    ...
    public async ValueTask FocusAndClick(ElementReference element)
    {
        IJSObjectReference module = await _moduleTask.Value;
        await module.InvokeVoidAsync("focusAndClick", element);
    }
    ...
    {
        IJSObjectReference module = await _moduleTask.Value;
        return await module.InvokeAsync<string>("getNotificationPermission");
    }
